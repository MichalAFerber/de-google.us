// Serve dist/ locally with the SAME headers Cloudflare Pages will apply in
// production — parsed straight out of dist/_headers — so the Playwright harness
// (§15) exercises the site under the real deployed CSP, not a permissive dev
// server. Also mimics Pages' 404.html behavior for unknown routes.
...
async function loadHeaders() {
  ...
  try {
    ...
    console.warn('[serve-with-csp] no dist/_headers found — serving without CSP');
  }
  return map;
}
...
}).listen(PORT, () => console.log(`[serve-with-csp] dist/ on http://localhost:${PORT} under production headers`));
